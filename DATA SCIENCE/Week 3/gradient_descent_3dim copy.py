...
def gradient_desc(m_now, b_now, data, L):
    ...
    for i in range(n):
        x = data[i][0]
        y = data[i][1]
        m_gradient += ((y - (m_now * x + b_now)) * x) / -n
        b_gradient += (y - (m_now * x + b_now)) / -n
...
for i in range(epochs):
    ...
    # ax.plot_surface(X, y, list(range(0, 10)), cmap='viridis')
    # # plt.plot(X, y, )
    # plt.scatter(m, b, color='red')
    # plt.pause(0.001)
    # plt.clf()
    print("Локальний мінімум має місце в", m, b)
print(m, b)
