...
async def get_contacts_by(name: str = Query(None, min_length=1, max_length=70),
                          ...
                          db: AsyncSession = Depends(get_db)):
    return await repository_contacts.get_contacts_by(name, surname, email, db)


@router.get("/birthdays", response_model=list[ContactResponse])
async def get_contacts_by(db: AsyncSession = Depends(get_db)):
    return await repository_contacts.get_contacts_birthdays(db)
...
async def populate_contact(count: int = Path(ge=1, le=150), db: AsyncSession = Depends(get_db)):
    return await repository_contacts.populate_contacts(count, db)
